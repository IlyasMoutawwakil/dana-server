...
function createSnapshotProjectId(projectId) {
  ...
  if (global.projects[projectId].infos.existsSync("tests.series")) {
    ...
    if (k.length !== 0) {
      ...
      if (global.projects[projectId].infos.existsSync("tests.statusSeries")) {
        statusSeries =
          global.projects[projectId].infos.readSync("tests.statusSeries");
      }
      let id = Object.keys(statusSeries).length;
      statusSeries[id] = res;
      global.projects[projectId].infos.writeSync(
        "tests.statusSeries",
        statusSeries
      );
    }
  ...
  if (global.projects[projectId].infos.existsSync("benchmarks.series")) {
    let benchmarks =
      global.projects[projectId].infos.readSync("benchmarks.series");
    let res = {
      numSeries: 0,
      numSeriesSimilar: 0,
      numSeriesImproved: 0,
    ...
    if (k.length !== 0) {
      ...
      let statusSeries = {};
      if (
        global.projects[projectId].infos.existsSync("benchmarks.statusSeries")
      ) {
        statusSeries = global.projects[projectId].infos.readSync(
          "benchmarks.statusSeries"
        );
      }
      let id = Object.keys(statusSeries).length;
      statusSeries[id] = res;
  ...
  for (let ii = 0; ii < k.length; ii++) {
    let compareId = k[ii];
    if (
      global.projects[projectId].infos.existsSync(
        "benchmarks.compare_" + compareId
      )
    ) {
      let compare;
      compare = global.projects[projectId].infos.readSync(
        "benchmarks.compare_" + compareId
      );
      let res = {
        numSeries: 0,
      ...
      if (
        global.projects[projectId].infos.existsSync(
          "benchmarks.statusCompare_" + compareId
        )
      ) {
        statusCompares = global.projects[projectId].infos.readSync(
          "benchmarks.statusCompare_" + compareId
        );
      }
...
function sendDailyReport(projectId) {
  ...
  moduleHtmlBuilder.bodyTableTd(["Project Id", "<b>" + projectId + "<b>"]);
  ...
  if (global.projects[projectId].infos.existsSync("tests.series")) {
    ...
    if (k.length !== 0) {
      ...
      for (let ii = 0; ii < k.length; ii++) {
        ...
        if (s.new === undefined) continue;
        delete s.new;

        if (st === "failingNeedstriage") {
          if (first) {
            moduleHtmlBuilder.bodyTableStart();
            moduleHtmlBuilder.bodyTableTh([
              "Id",
              "Last Passing",
              "Failing Since",
              "Last Executed",
          ...
          moduleHtmlBuilder.bodyTableTd([
            ...
            "<a href=" +
              ...
              projectId +
              "?" +
              encodeURIComponent(k[ii]) +
              ">View</a>",
          ]);
        }
      }
      if (!first) moduleHtmlBuilder.bodyTableEnd();
      else moduleHtmlBuilder.bodyPar("None");
        ...
        for (let ii = 0; ii < dataBenchmarksNew.length; ii++) {
          ...
          moduleHtmlBuilder.bodyTableTd([
            ...
            "<a href=" +
              ...
              projectId +
              "?" +
              encodeURIComponent(dataBenchmarksNew[ii].serieId) +
              ">View</a>",
          ]);
        }
        moduleHtmlBuilder.bodyTableEnd();
    ...
    if (
      ...
      if (dataCompareNew.length !== 0) {
        ...
        for (let jj = 0; jj < dataCompareNew.length; jj++) {
          ...
          if (s.result.diff == 0) ratio = "0%";
          if (s.result.diff > 0) ratio = "+" + p.toFixed(2) + "%";
          if (s.result.diff < 0) ratio = p.toFixed(2) + "%";
          moduleHtmlBuilder.bodyTableTd([
            ...
            "<a href=" +
              ...
              projectId +
              "?" +
              encodeURIComponent(dataCompareNew[jj].serieId) +
              ">View</a>",
          ]);
        }
        moduleHtmlBuilder.bodyTableEnd();
  ...
  moduleHtmlBuilder.bodyPar(
    "<a href=" + danaUrl + "><i>-- Your Dana dashboard </i><a>"
  );
  ...
  fs.writeFileSync(
    cwd + "/www/public/dailyReports/" + projectId + ".html",
    "<H3>" + subject + "<br></H3>"
  );
  fs.appendFileSync(
    cwd + "/www/public/dailyReports/" + projectId + ".html",
...
function apiAddBuild(apiData, hdl) {
  ...
  if (builds[build.buildId] && apiData.override !== true) {
    return hdl(
      "build.buildId already exist, use override flag to override content."
    );
  }
...
function apiAddSerie(apiData, hdl, apiVersion) {
  ...
  if (!newSerie && apiData.override !== true) {
    return hdl(
      "Serie serieId already exist, use override flag to override content."
    );
  }
...
function apiAddSample(apiData, hdl, apiVersion) {
  ...
  for (let ii = 0; ii < samples.length; ii++) {
    ...
    if (serie.samples !== undefined) {
      if (serie.samples[sample.buildId] && apiData.override !== true) {
        return hdl(
          "Sample already exist, use override flag to override content."
        );
      }
    }
    ...
    if (serie.analyse.benchmark) {
      ...
      //
      let convertedValue = moduleUtils.unitConversion(
        sample.value,
        sampleUnit,
        serie.serieUnit
      );
  ...
  if (serie.analyse.benchmark) {
    ...
    if (apiData.skipAnalysis !== true) {
      ...
      if (serie.analyseResult.summary.reduceBaseTo) {
        if (global.debug)
          console.log(
            "Reset base to",
            serie.analyseResult.summary.reduceBaseTo,
            serieId
          );
      ...
      if (st !== "similarNoisy") {
        if (serie.analyseResult.summary.status === "regression") {
          ...
          if (serie.analyseResult.summary.status === "improvement") {
            ...
            if (st === undefined) serie.state.analyse = "similarNeedstriage";
    ...
    for (let ii = 0; ii < kk.length; ii++) {
      ...
      if (cp.filter) if (serieId.indexOf(cp.filter) === -1) continue;

      let cpSerie;
      let cpSerieId = serieId;

      if (cp.compareWith.replace) {
        cpSerieId = cpSerieId.replace(
          cp.compareWith.replace.substr,
          cp.compareWith.replace.newSubStr
        );
      }
...
if (!fs.existsSync(cwd + "/configs/gmail-credentials.json")) {
  console.log(
    "configs/gmail-credentials.json not found, disabling email reports"
  );
  global.emailIsEnabled = false;
}
...
for (let ii = 0; ii < k.length; ii++) {
  ...
  global.projects[k[ii]].series = new ModuleFiles(
    cwd + "/configs/db/" + k[ii] + "/series",
    200
  );
  global.projects[k[ii]].comments = new ModuleFiles(
    cwd + "/configs/db/" + k[ii] + "/comments",
    200
  );
  global.projects[k[ii]].infos = new ModuleFiles(
    cwd + "/configs/db/" + k[ii] + "/infos",
    200
  );
}
...
app.get(
  ...
  function (req, res) {
    if (global.config.defaultUrl) res.redirect(global.config.defaultUrl);
    else res.redirect("/admin/viewProjects");
  }
);
...
app.get(
  ...
  function (req, res) {
    ...
    } else if (r.indexOf("deleteProject") !== -1) {
      r = r.split("?");
      let projectId = r[1];
      console.log(global.projectsConfig[projectId]);
    ...
    } else if (r.indexOf("editCompare") !== -1) {
      r = r.split("?");
      let compareId = r[1];
    ...
    } else if (r.indexOf("deleteCompare") !== -1) {
      r = r.split("?");
      let compareId = r[1];
      console.log(global.comparesConfig[compareId]);
...
app.post(
  ...
  require("connect-ensure-login").ensureLoggedIn(),
  function (req, res) {
    if (global.debug) console.log("/admin/addComparator", req.body);
    ...
    if (global.comparesConfig[compareId] !== undefined) {
      appError(
        req,
        res,
        "/admin/addComparator, comparator " + compareId + " already exist"
      );
      return;
    }
    ...
    if (compareWith.projectId === "") {
      appError(
        req,
        res,
        "/admin/addComparator, compareWith.projectId undefined"
      );
      return;
    }
    ...
    if (req.body.with_buildId !== "None")
      compareWith.buildId = req.body.with_buildId * 1;

    compareWith.useAverage = true;
    if (req.body.with_useAverage !== "with_useAverage")
      compareWith.useAverage = false;
...
app.post(
  "/admin/saveComparator",
  require("connect-ensure-login").ensureLoggedIn(),
  function (req, res) {
    if (global.debug) console.log("/admin/saveComparator", req.body);
    ...
    if (global.comparesConfig[compareId] === undefined) {
      appError(
        req,
        res,
        "/admin/saveComparator, comparator " + compareId + " doesnt exist"
      );
      return;
    }
    ...
    if (compareWith.projectId === "") {
      appError(
        req,
        res,
        "/admin/saveComparator, compareWith.projectId undefined"
      );
      return;
    }
...
app.post(
  "/admin/deleteComparator",
  require("connect-ensure-login").ensureLoggedIn(),
  function (req, res) {
    if (global.debug) console.log("/admin/deleteComparator", req.body);
    ...
    if (global.comparesConfig[compareId] === undefined) {
      appError(
        req,
        res,
        "/admin/deleteComparator, comparator " + compareId + " doesnt exist"
      );
      return;
    }
  ...
  function (req, res) {
    ...
    if (global.projectsConfig[projectId] !== undefined) {
      appError(
        req,
        res,
        "/admin/addProject, project of " + req.body.projectId + " already exist"
      );
      return;
    }
  ...
  function (req, res) {
    ...
    if (global.projectsConfig[projectId] === undefined) {
      appError(
        req,
        res,
        "/admin/saveProject, project of " + projectId + " doesnt exist"
      );
      return;
    }
  ...
  function (req, res) {
    ...
    if (global.projectsConfig[projectId] === undefined) {
      appError(
        req,
        res,
        "/admin/deleteProject, project of " + projectId + " doesnt exist"
      );
      return;
    }
...
app.get("/serie", function (req, res) {
  ...
  if (global.projects[projectId] === undefined) {
    console.error(
      "/serie projectId  not in projects",
      projectId,
      global.projects
    );
    return;
...
app.post("/apis/*", function (req, res, next) {
  if (!req.is("application/json")) {
    console.log("Expected application/json Content-Type");
  ...
  if (authType !== "Bearer") {
    console.log("Expected a Bearer token");
...
server.listen(global.config.server.port, function () {
  console.log(
    "Dana web server listening at port %d",
...
io.on("connection", function (socket) {
  ...
  socket.on("getOneSerie", function (req) {
    ...
    if (global.debug)
      console.log("getOneSerie query", serieId, "result", req.serie);
  ...
  socket.on("serieUpdateAnalyse", function (req) {
    ...
    if (serie.analyse.benchmark) {
      ...
      if (st !== "similarNoisy") {
        if (serie.analyseResult.summary.status === "regression") {
          if (st === undefined) serie.state.analyse = "regressionNeedstriage";
          else if (st.indexOf("regression") === -1)
            serie.state.analyse = "regressionNeedstriage";
        } else {
          serie.assignee.analyse = undefined;
      ...
      for (let ii = 0; ii < k.length; ii++) {
        ...
        if (result !== undefined) {
          ...
          // compares file
          let compare = global.projects[projectId].infos.readSync(
            "benchmarks.compare_" + compareId
          );
  ...
  socket.on("serieUpdateAssignee", function (req) {
    ...
    if (compareId === undefined) {
      ...
      if (serie.assignee.analyse !== assignee) {
        ...
        if (serie.assignee.analyse) t += serie.assignee.analyse;
        else t += "None";
        t += " -> ";
      ...
      if (!global.comparesConfig[compareId]) {
        let e =
          "socket serieUpdateAssignee compareId compares doesnt exist " +
          compareId;
        console.error(e, projectId, serieId);
        socket.emit("serverError", e);
        return;
      ...
      if (serie.assignee.compares[compareId] !== assignee) {
        t += "<i>Assignee for " + compares[compareId].description + " updated ";
  ...
  socket.on("serieUpdateSeriesState", function (req) {
    ...
    if (!compareId) {
      ...
      if (serie.state.analyse !== state) {
        t +=
          "<i>Regression analysis state updated " +
          serie.state.analyse +
          " -> " +
          state +
          "</i><br>";
        if (serie.analyse.benchmark) {
          if (state === "regressionNoisy") state = "similarNoisy";
          if (state === "improvementNoisy") state = "similarNoisy";
        }
      ...
      if (!global.comparesConfig[compareId]) {
        let e =
          "socket serieUpdateState compareId compares doesnt exist " +
          compareId;
        console.error(e, projectId, serieId);
        socket.emit("serverError", e);
        return;
  ...
  socket.on("serieUpdateBugLink", function (req) {
    ...
    if (!compareId) {
      ...
      if (!global.comparesConfig[compareId]) {
        let e =
          "socket serieUpdateBugLink compareId compares doesnt exist " +
          compareId;
        console.error(e, projectId, serieId);
        socket.emit("serverError", e);
        return;
